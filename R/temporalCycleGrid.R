...
#' @description A function to compute a \dQuote{daily/monthly climatology}. See Details.
...
#' @param aggr.fun Function used to aggregate the values. Default to \code{\link{mean} }.
#' @param ... Further arguments passed to \code{aggr.fun} (e.g. \code{na.rm = TRUE}, ...)
...
#' recommended the application of \code{\link{filterGrid}} prior to its usage (See examples).
#' 
...
#' plot(Mn.agg$Data, ty = 'l', ylim = c(0,15), axes = FALSE, xlab = "month-day", ylab = "Mean Temp (degC)")
#' axis(2)
...
#' polygon(x = c(ix, rev(ix)), y = c(mn.agg$Data, rev(mx.agg$Data)), border = "transparent", col = rgb(.2,.2,.2,.5))
#' legend("top", c("Daily mean cycle", "range"),
#'        pch = 22, pt.bg = c("transparent", "grey60"), pt.cex = c(0,2.5),
#'        lwd = c(1,0), bty = "n")
#' title(main = "Daily Cycle DJF 1997-2000")
#' 
#' ## Smoothing daily data
#' ## A moving window of 1 week smoothes the daily series prior to annual cycle calculation:
#' fgrid <- filterGrid(grid, window.width = 7, sides = 2)
#' fMn <- temporalCycleGrid(fgrid, time.frame = "daily")
...
temporalCycleGrid <- function(grid, time.frame = c("daily", "monthly"),
    ...
    if (time.frame == "daily") {
        ...
        if (tindex %>% diff() %>% head(1) == 0) stop("The input grid is not monthly\nConsider the application of 'aggregateGrid' first")
    }
